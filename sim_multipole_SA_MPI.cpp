...
int main(int argc, char *argv[])
{
  ...
  const uint num_SA_steps = strtol(argv[7], NULL, 10);
  const uint num_sweeps_SA = pow(10, strtol(argv[8], NULL, 10));
  double cooling_rate = 0.9;
  double initial_T = 1*abs(jtau);
  double final_T = pow(cooling_rate,num_SA_steps);

  //deterministic sweep settings
  const uint max_det_sweeps = pow(10,strtol(argv[9], NULL, 10));
